...
(function () {
  ...
  function wallpaper(hex) {
    ...
    diamond(8, 8, shade(hex, -38));
    diamond(24, 24, shade(hex, -38));
    diamond(8, 8 + 16, shade(hex, -22));
    diamond(24, 24 - 16, shade(hex, -22));
  ...
  function floorboards(hex) {
    ...
    g.fillStyle = shade(hex, -8); g.fillRect(0, 0, S, S);

    var rows = 4, hgt = S / rows;
    for (var r = 0; r < rows; r++) {
      var y = r * hgt;
      // board face, alternating tone
      g.fillStyle = shade(hex, [7, -5, 2, -11][r]);
      g.fillRect(0, y, S, hgt - 1);
      // seam between boards
      g.fillStyle = shade(hex, -48);
      g.fillRect(0, y + hgt - 1, S, 1);
      // staggered end joints
      var joint = (r % 2 ? 20 : 44);
      g.fillRect(joint, y, 1, hgt - 1);
      // grain
      g.fillStyle = shade(hex, -24);
      for (var i = 0; i < 5; i++) {
        var gx = (r * 13 + i * 11) % S;
        g.fillRect(gx, y + 2 + (i % 3) * 3, 7, 1);
      }
      g.fillStyle = shade(hex, 16);
      g.fillRect((r * 7 + 5) % S, y + 5, 9, 1);
    }

    /* three tiles across, nine along — long narrow boards, not floor tiles */
    var t = pixelTexture(c, 3, 9);
    floorCache[hex] = t;
    return t;
  }
